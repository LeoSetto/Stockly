...
{(()=>{if((form.splitPayers||[]).length===0)return null;const splitDiff=Math.abs((Number(form.splitMyShare)||0)+(form.splitPayers||[]).reduce((a,p)=>a+p.amount,0)-(Number(form.splitTotal)||0));return(<div style={{fontSize:13,marginTop:8,padding:"8px 12px",borderRadius:8,background:"var(--purple-bg)",color:"var(--purple)"}}>Total: {fmt(Number(form.splitTotal)||0)} = {myName}: {fmt(Number(form.splitMyShare)||0)}{(form.splitPayers||[]).map(p=>` + ${p.name}: ${fmt(p.amount)}`).join("")}{splitDiff>0.01&&<span style={{color:"var(--red)",fontWeight:600}}> ⚠ Os valores não batem!</span>}</div>);})()}
</div>}</div>
