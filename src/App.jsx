...
.app{display:flex;min-height:100vh}.sb{width:260px;min-height:100vh;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;position:fixed;top:0;left:0;z-index:100;transition:transform .3s}.sb-h{padding:28px 24px 20px;border-bottom:1px solid var(--border)}.logo{font-family:'Sora',sans-serif;font-size:26px;font-weight:800;background:linear-gradient(135deg,var(--accent),#FFD700);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:-.5px}.logo-s{font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:3px;margin-top:4px;font-weight:500}
...
.mc{flex:1;margin-left:260px;padding:32px 40px;max-width:1200px}.ph{margin-bottom:32px}.pt{font-family:'Sora',sans-serif;font-size:32px;font-weight:800;color:var(--text);letter-spacing:-.5px}.ps{color:var(--text3);font-size:14px;margin-top:6px}
...
input,select,textarea{background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:10px 14px;color:var(--text);font-size:14px;font-family:'DM Sans',sans-serif;outline:none;transition:border-color .2s;width:100%}input:focus,select:focus,textarea:focus{border-color:var(--accent)}select{cursor:pointer;-webkit-appearance:none}textarea{resize:vertical;min-height:60px}
...
{(()=>{if((form.splitPayers||[]).length===0)return null;const splitDiff=Math.abs((Number(form.splitMyShare)||0)+(form.splitPayers||[]).reduce((a,p)=>a+p.amount,0)-(Number(form.splitTotal)||0));const mismatch=splitDiff>0.01;return(<div style={{fontSize:13,marginTop:8,padding:"8px 12px",borderRadius:8,background:"var(--purple-bg)",color:"var(--purple)"}}>Total: {fmt(Number(form.splitTotal)||0)} = {myName}: {fmt(Number(form.splitMyShare)||0)}{(form.splitPayers||[]).map(p=>` + ${p.name}: ${fmt(p.amount)}`).join("")}{mismatch&&<span style={{color:"var(--red)",fontWeight:600}}> ⚠ Os valores não batem!</span>}</div>);})()}
</div>}</div>
