...
.app{display:flex;min-height:100vh;overflow-x:hidden}.sb{width:260px;height:100vh;background:var(--bg2);border-right:1px solid var(--border);display:flex;flex-direction:column;position:fixed;top:0;left:0;z-index:100;transition:transform .3s cubic-bezier(.4,0,.2,1);overflow-y:auto;scrollbar-width:none}.sb::-webkit-scrollbar{display:none}.sb-h{padding:28px 24px 20px;border-bottom:1px solid var(--border);flex-shrink:0}.logo{font-family:'Sora',sans-serif;font-size:26px;font-weight:800;background:linear-gradient(135deg,var(--accent),#FFD700);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:-.5px;transition:opacity .3s}.logo:hover{opacity:.85}.logo-s{font-size:11px;color:var(--text3);text-transform:uppercase;letter-spacing:3px;margin-top:4px;font-weight:500}
.nav{padding:16px 12px;flex:1;display:flex;flex-direction:column;gap:4px;overflow-y:auto;min-height:0;scrollbar-width:none}.nav::-webkit-scrollbar{display:none}.ni{display:flex;align-items:center;gap:12px;padding:12px 16px;border-radius:10px;cursor:pointer;transition:all .2s cubic-bezier(.4,0,.2,1);color:var(--text2);font-size:14px;font-weight:500;border:none;background:none;width:100%;text-align:left;font-family:inherit;position:relative}.ni:hover{background:var(--bg3);color:var(--text);transform:translateX(4px)}.ni:active{transform:translateX(2px) scale(.98)}.ni.a{background:var(--accent-glow);color:var(--accent);font-weight:600}.ni.a svg{stroke:var(--accent)}.ni.a::after{content:'';position:absolute;left:0;top:25%;bottom:25%;width:3px;background:var(--accent);border-radius:0 3px 3px 0}.nb{margin-left:auto;background:var(--red);color:#fff;font-size:11px;font-weight:700;padding:2px 7px;border-radius:20px;min-width:20px;text-align:center;animation:pulse 2s infinite}.sb-f{padding:12px 16px;border-top:1px solid var(--border);font-size:11px;color:var(--text3);flex-shrink:0}
@keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}
.mc{flex:1;margin-left:260px;padding:32px 40px;max-width:1200px;animation:pageIn .3s ease;overflow-x:hidden}.ph{margin-bottom:32px}.pt{font-family:'Sora',sans-serif;font-size:32px;font-weight:800;color:var(--text);letter-spacing:-.5px}.ps{color:var(--text3);font-size:14px;margin-top:6px}
...
.cb-c{display:flex;align-items:flex-end;gap:8px;height:120px;padding-top:10px;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}.cb-c::-webkit-scrollbar{display:none}.cb-col{flex:1;min-width:56px;display:flex;flex-direction:column;align-items:center;gap:6px;height:100%;justify-content:flex-end;transition:all .2s}.cb-col:hover{transform:translateY(-2px)}.cb-b{width:100%;max-width:48px;border-radius:6px 6px 0 0;transition:height .6s cubic-bezier(.4,0,.2,1);min-height:4px}.cb-l{font-size:10px;color:var(--text3);text-align:center;max-width:64px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.cb-v{font-size:11px;color:var(--text2);font-weight:600;white-space:nowrap}
...
// Bulk select
const[bulkMode,setBulkMode]=useState(false);const[bulkSel,setBulkSel]=useState({});
const toggleBulk=(id)=>setBulkSel(s=>({...s,[id]:!s[id]}));
const selCount=Object.values(bulkSel).filter(Boolean).length;
const selectAll=(list)=>{const s={};list.forEach(i=>s[i.id]=true);setBulkSel(s);};
const deselectAll=()=>setBulkSel({});
const bulkDelete=()=>{const ids=Object.keys(bulkSel).filter(k=>bulkSel[k]);if(ids.length===0)return;setData(d=>({...d,grocery:d.grocery.filter(i=>!ids.includes(String(i.id)))}));toast(`${ids.length} ite${ids.length>1?"ns":"m"} removido${ids.length>1?"s":""}`);setBulkSel({});setBulkMode(false);};
// Autocomplete
const allNames=[...new Set([...data.grocery.map(i=>i.name),...data.pantry.map(i=>i.name),...(data.priceHistory||[]).map(p=>p.name)])].sort();
...
const mainAmount=doneTotal-splitTotal;
...
<div className="tb"><button className="btn bp" onClick={()=>{setForm({name:"",qty:"",unit:c.units[0],category:c.pantryCategories[0]});setSuggestions([]);setModal(true);}}>{I.plus} Adicionar</button>
...
{(pend.length>0||done.length>0)&&<button className={`btn ${bulkMode?"bp":"bg"} bs`} onClick={()=>{setBulkMode(!bulkMode);setBulkSel({});}}>{bulkMode?"✕ Sair":"☑ Selecionar"}</button>}
{done.length>0&&!bulkMode&&<button className="btn bg" onClick={openFinish}>Finalizar Compra ({fmt(doneTotal)})</button>}</div>
{bulkMode&&<div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
<button className="btn bg bs" onClick={()=>selectAll([...pend,...done])}>Selecionar Tudo ({pend.length+done.length})</button>
<button className="btn bg bs" onClick={()=>selectAll(pend)}>Pendentes ({pend.length})</button>
<button className="btn bg bs" onClick={()=>selectAll(done)}>Comprados ({done.length})</button>
<button className="btn bg bs" onClick={deselectAll}>Limpar</button>
{selCount>0&&<button className="btn bd bs" onClick={bulkDelete}>{I.trash} Apagar {selCount}</button>}
</div>}
...
<div className="fr"><div className="fg" style={{flex:2,position:"relative"}}><label className="fl">Nome</label><input value={form.name||""} onChange={e=>onNameChange(e.target.value)} autoFocus autoComplete="off"/>
{suggestions.length>0&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:"var(--bg2)",border:"1px solid var(--border2)",borderRadius:8,zIndex:10,maxHeight:160,overflowY:"auto",boxShadow:"var(--shadow)"}}>{suggestions.map((s,i)=>(<div key={i} style={{padding:"10px 14px",cursor:"pointer",fontSize:13,borderBottom:"1px solid var(--border)",transition:"background .1s"}} onClick={()=>{setForm({...form,name:s});setSuggestions([]);}} onMouseDown={e=>e.preventDefault()}>{s}</div>))}</div>}
...
<div style={{fontSize:24,fontWeight:800,color:"var(--accent)",textAlign:"center",marginBottom:4}}>{fmt(doneTotal)}</div>
...
<div className="fg" style={{marginBottom:12}}><label className="fl">Meu cartão / pagamento</label>
...
<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,cursor:"pointer"}} onClick={()=>setFinishPaid(!finishPaid)}>
...
<span style={{fontSize:14,color:finishPaid?"var(--green)":"var(--yellow)",fontWeight:500}}>{finishPaid?"Já pago":"Pendente"}</span>
</div>
{/* Split section */}
...
// By card — separate paid and pending
const byCard={};const byCardPaid={};const byCardPending={};
mExpenses.forEach(e=>{const k=e.card||"Sem cartão";const amt=getMyAmount(e);byCard[k]=(byCard[k]||0)+amt;if(e.paid){byCardPaid[k]=(byCardPaid[k]||0)+amt;}else{byCardPending[k]=(byCardPending[k]||0)+amt;}});
// Income by category
...
{/* Totals by card */}
{Object.keys(byCard).length>0&&<div className="card"><div className="ct">💳 Totais por Cartão</div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}>{Object.entries(byCard).sort((a,b)=>b[1]-a[1]).map(([card,val],idx)=>{const paid=byCardPaid[card]||0;const pend=byCardPending[card]||0;return(<div key={card} style={{background:"var(--bg3)",borderRadius:10,padding:"12px 14px",flex:"1 1 calc(50% - 10px)",minWidth:0,borderLeft:`4px solid ${cardColors[idx%cardColors.length]}`}}><div style={{fontSize:11,color:"var(--text3)",textTransform:"uppercase",letterSpacing:1,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{card}</div><div style={{fontSize:18,fontWeight:700,marginTop:2}}>{fmt(val)}</div><div style={{fontSize:10,marginTop:2,display:"flex",gap:8}}>{paid>0&&<span style={{color:"var(--green)"}}>✓ {fmt(paid)}</span>}{pend>0&&<span style={{color:"var(--yellow)"}}>◷ {fmt(pend)}</span>}</div></div>);})}</div></div>}
{/* Expenses list */}
